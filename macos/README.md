...
- 适合 macOS 用户（macOS 13 或更新）。
- 需要能运行本机 `.app`，或能在终端里运行 `./build.sh`。
- 聊天需要用户自己的 DeepSeek API Key。
- “看屏幕/停下”需要手动开启屏幕录制权限，并配一个本地大模型做脱敏。
...
DeepSeek API Key 保存在系统钥匙串里，通过 macOS Security 框架直接写入，key 不会作为命令行参数出现在进程列表中。

## 看屏幕的本地脱敏

看屏幕是两段式的，屏幕原文不会直接发往云端：

1. 截图后在本机用 Apple Vision 做 OCR。
2. OCR 文字先送到你自己的本地大模型脱敏，删除密码、验证码、账号、手机号、邮箱等敏感信息。
3. 只有脱敏后的文字才发给云端 DeepSeek，让熊说一句短话。

本地模型连不上时，这一轮不会把原始屏幕文字发往云端（fail-closed）。

默认走 Ollama 的 OpenAI 兼容接口，2060 + 16G 跑 3B 量化模型足够：
...
默认地址 `http://localhost:11434/v1/chat/completions`，默认模型 `qwen2.5:3b`。想改地址或模型名，用菜单 **设置本地模型**（`⌘G`），或 **查看本地模型** 看当前配置。任何 OpenAI 兼容的本地服务都可以。
